...
import React, { useState, useEffect, useRef } from 'react';
import Plot from 'react-plotly.js';
import { Shield, ShieldAlert, Zap, BrainCircuit, RefreshCw } from 'lucide-react'; 
import './App.css';

function App() {
  const [current, setCurrent] = useState({ speed: 0, status: "STANDBY", is_danger: false, reason: "System Ready." });
  const [loading, setLoading] = useState(false);
  const [logs, setLogs] = useState([]);

  const triggerAttack = async () => {
    setLoading(true); // UI foran respond karegi
    try {
      const response = await fetch("http://127.0.0.1:8000/trigger-attack", { method: "POST" });
      const data = await response.json();
      setCurrent(data);
      setLogs(prev => [`> [CRITICAL] ${data.status} detected`, ...prev]);
    } catch (error) {
  ...
  return (
    <div className={`app-container ${current.is_danger ? 'alert-mode' : ''}`}>
      <nav className="navbar">
        <div className="logo"><Shield size={24}/> V2V SENTINEL <span className="xai">XAI</span></div>
      ...
      <div className="main-grid">
        {/* Left Side: Live Radar */}
        <div className="glass-panel radar-section">
          <h3><Zap size={16}/> LIVE SPATIAL ANALYSIS</h3>
          <div className="radar-placeholder">
             {/* Plotly Chart Yahan Ayega */}
          </div>
        </div>

        {/* Right Side: AI Reasoning */}
        <div className="glass-panel ai-section">
          <h3><BrainCircuit size={16}/> AGENTIC REASONING</h3>
          <div className="ai-box">
            {loading ? (
              <p className="typing-effect">AI Agent is evaluating network packets...</p>
            ) : (
              <p className="reason-text">{current.reason}</p>
            )}
          </div>
          <button className="btn-simulate" onClick={triggerAttack} disabled={loading}>
            {loading ? "ANALYZING..." : "SIMULATE ATTACK"}
          </button>
        </div>
      </div>

      {/* FOOTER: Training Proofs */}
      <footer className="footer">
        <div className="proof-card">
          <span>Model Accuracy</span>
          <img src="/accuracy_plot.png" alt="Accuracy" />
        </div>
        <div className="proof-card">
          <span>SHAP Analysis</span>
          <img src="/loss_plot.png" alt="SHAP" />
        </div>
      </footer>
    </div>
  );
